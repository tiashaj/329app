...
class ShortsViewController: UIViewController, UICollectionViewDataSource {
    ...
    func loadDB(){
        ...
        ref.child("Users").child(userID!).child("shorts").observe(DataEventType.value) { (snapshot) in
            var newImages = [Pic]()
