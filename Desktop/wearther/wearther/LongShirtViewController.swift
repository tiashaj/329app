...
class LongShirtViewController: UIViewController, UICollectionViewDataSource {
    ...
    func loadDB(){
        ...
        ref.child("Users").child(userID!).child("long_sleeves").observe(DataEventType.value) { (snapshot) in
            var newImages = [Pic]()
